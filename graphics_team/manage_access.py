...
class GraphicsTeam_Render_Access:
    ...
    def access_for_certificates(request):
        ''' This function checks if the requested user has access to create/edit certificates. Will return True if it has access permission '''
        try:
            # get the user and username. Username will work as IEEE ID and Developer username both
            user=request.user
            username=user.username
            ...
            if(get_member.exists()):
                #The member exists. Now check if it has create/edit graphics access
                if(get_member[0].certificates_access or GraphicsTeam_Render_Access.get_common_access(request)):
                    return True
                else:
                    return False
            else:
    ...
    def access_for_view_certificates(request):
        ''' This function checks if the requested user has access to view certificates. Will return True if it has access permission '''
        try:
            # get the user and username. Username will work as IEEE ID and Developer username both
            user=request.user
            username=user.username
            ...
            if(get_member.exists()):
                #The member exists. Now check if it has only view graphics access
                if(get_member[0].certificates_view_access or GraphicsTeam_Render_Access.get_common_access(request)):
                    return True
                else:
                    return False
            else:
